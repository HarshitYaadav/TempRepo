...
import pickle
from rnn_model import RNNForecaster, LSTMForecaster, GRUForecaster, RNNTrainer


def get_model(model_type, input_size, hidden_size=64, num_layers=2, dropout=0.2):
    """Factory function to get the appropriate model"""
    if model_type == 'rnn':
        return RNNForecaster(input_size, hidden_size, num_layers, dropout)
    elif model_type == 'lstm':
        return LSTMForecaster(input_size, hidden_size, num_layers, dropout)
    elif model_type == 'gru':
        return GRUForecaster(input_size, hidden_size, num_layers, dropout)
    else:
...
def train_single_model(df, target_column, index_name, model_type='rnn',
                       SEQUENCE_LENGTH=12, HIDDEN_SIZE=64, NUM_LAYERS=2, 
                       DROPOUT=0.2, BATCH_SIZE=32, EPOCHS=100, LEARNING_RATE=0.001,
                       TRAIN_SPLIT_RATIO=0.8):
    """
    Train a single model for a specific WSI index
    
    Args:
        df: DataFrame with data
        target_column: Target column name
        index_name: Name of the index ('entropy', 'equal', 'pca', 'hybrid')
        model_type: Type of model ('rnn', 'lstm', 'gru')
        Other parameters: Model and training hyperparameters
    """
    print(f"\n{'#'*60}")
    print(f"# Training {model_type.upper()} Model for WSI {index_name.upper()}")
    print(f"{'#'*60}")
    ...
    trainer.save_model(f'{model_type}_model_{index_name}.pth')
    
    # Save scalers
    with open(f'{model_type}_scalers_{index_name}.pkl', 'wb') as f:
        pickle.dump(scalers, f)
    print(f"Scalers saved to {model_type}_scalers_{index_name}.pkl")
    
    # Save feature columns
    with open(f'{model_type}_feature_cols_{index_name}.pkl', 'wb') as f:
        pickle.dump(feature_cols, f)
    print(f"Feature columns saved to {model_type}_feature_cols_{index_name}.pkl")
    
    # Save split information
    with open(f'{model_type}_split_info_{index_name}.pkl', 'wb') as f:
        pickle.dump(split_info, f)
    print(f"Split info saved to {model_type}_split_info_{index_name}.pkl")
...
def main():
    """Main training function - trains all models for all WSI indices"""
    print("\n" + "="*60)
    print("MULTI-MODEL TRAINING FOR WATER STRESS INDEX PREDICTION")
    print("="*60)
    ...
    # Define model types and indices
    ...
    print(f"  - WSI indices: {', '.join([i[1].upper() for i in indices])}")
    print(f"  - Total models to train: {len(model_types) * len(indices)}")
    ...
    for model_type in model_types:
        for target_column, index_name in indices:
            key = f"{model_type}_{index_name}"
            
            history, model, trainer = train_single_model(
                df, 
    ...
    for model_type in model_types:
        ...
        for _, index_name in indices:
            key = f"{model_type}_{index_name}"
            res = results[key]
            print(f"  ✓ {index_name.upper()}: Train Loss={res['final_train_loss']:.6f}, Val Loss={res['final_val_loss']:.6f}, Epochs={res['epochs']}")
