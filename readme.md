...
- **GRU** generally performs best across all indices (lowest RMSE and MAE)
- **RNN** achieves highest R² on PCA and Entropy indices
- **LSTM** shows balanced performance but doesn't consistently excel
- **PCA-based WSI** achieves highest R² but higher MAPE (due to scale differences)
- **Equal-weighted WSI** has most stable predictions (lowest MAPE among practical indices)
...
2. Check model-specific documentation (RNN_README.md)
3. Examine validation results in CSV files
4. Review visualization plots for insights
